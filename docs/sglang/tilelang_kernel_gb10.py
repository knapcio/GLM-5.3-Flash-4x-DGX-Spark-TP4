...
def sparse_attention_fwd_kernel_v1(
    ...
    *,
    ...
    import sys; open("/state/gb10-tiles.log","a").write(f"[gb10-tiles] v1 heads={num_heads} H_per_block={H_per_block} D={D} BI={BI} NI={NI} stages={num_stages} threads={threads}\n")
...
def sparse_attention_fwd_kernel_v2(
    ...
    *,
    ...
    H_per_block = padded_H if REPLICATE_H == 1 else 64
    import sys; open("/state/gb10-tiles.log","a").write(f"[gb10-tiles] v2 heads={num_heads} H_per_block={H_per_block} D={D} D_tail={D_tail} BI={BI} NI={NI} threads={threads}\n")
...
def tilelang_sparse_fwd(
    ...
    if _is_hip:
        ...
        import sys; open("/state/gb10-tiles.log","a").write(f"[gb10-tiles] sparse_fwd heads={num_heads} dim={dim} d_v={d_v} tail={tail_dim} topk={topk} factory={tail_dim == 0}\n")
