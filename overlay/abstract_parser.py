...
class DelegatingParser(Parser):
    ...
    # Local K6 literal-no-tools fix, 2026-09-08: both facade engines.
    def _configure_glm_literal_tools(self, request) -> None:
        if request.tool_choice == "none" or not request.tools:
            for adapter in (self._reasoning_parser, self._tool_parser):
                engine = getattr(adapter, "_parser_engine", None)
                if getattr(getattr(engine, "parser_engine_config", None), "name", None) == "glm47_moe":
    ...
    def parse(
        ...
        self._configure_glm_literal_tools(request)
