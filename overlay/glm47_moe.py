...
class Glm47MoeParser(ParserEngine):
    ...
    # Local K6 literal-no-tools fix, 2026-09-08
    def _check_skip_tool_parsing(self, request) -> None:
        if getattr(request, "tool_choice", None) == "none" or not getattr(request, "tools", None):
            if not getattr(self, "_literal_tools_disabled", False):
                from dataclasses import replace
                from vllm.parser.engine.streaming_parser_engine import StreamingParserEngine
                config = self.parser_engine_config
                keep = {"THINK_START", "THINK_END"}
                config = replace(config,
                    terminals={k:v for k,v in config.terminals.items() if k in keep},
                    token_id_terminals={k:v for k,v in config.token_id_terminals.items() if k in keep},
                    transitions={k:v for k,v in config.transitions.items() if k[1] in keep})
                self.parser_engine_config = config
                self._engine = StreamingParserEngine(config, self.model_tokenizer, initial_state=self._engine.state, vocab=self.vocab)
                self._literal_tools_disabled = True
