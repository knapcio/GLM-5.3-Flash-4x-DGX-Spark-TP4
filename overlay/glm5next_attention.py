...
class Glm5NextTailCache(DeepseekV32IndexerCache):
    ...
    def get_kv_cache_spec(self, vllm_config: VllmConfig):
        ...
        # Drafts are stashed before acceptance. With a one-pool ring the drafts
        # behind a rejected pool-completing draft overwrite the committed keys
        # its redo reads (vllm#58454): size the ring to hold kpool + num_spec.
        ...
        if not getattr(Glm5NextTailCache, "_glm_kpool_logged", False):
            Glm5NextTailCache._glm_kpool_logged = True
            import sys as _sys
            _sys.stderr.write(
                f"glm-kpool-fix: tail ring {ring} slots (kpool {self._index_kpool}, "
                f"num_spec {vllm_config.num_speculative_tokens})\n"
            )
        assert self.cache_config.block_size % ring == 0, (
            f"Glm5NextTailCache: block_size {self.cache_config.block_size} "
