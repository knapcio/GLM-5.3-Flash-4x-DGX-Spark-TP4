...
def _try_load_fp8_attn_proj(
    ...
    # quant-mix guard: when the model built this projection quantized (ModelOpt FP8/MXFP8,
    # compressed-tensors FP8 block, ...), its weight param is not BF16 and it expects the fp8
    # tensor as-is. Without this the fp8 weight is buffered waiting for a weight_scale_inv that
    # never arrives and the param stays uninitialized.
    _tp = params_dict.get(target_w)
    if _tp is not None and _tp.dtype not in (torch.bfloat16, torch.float16, torch.float32):
        return False
