...
def _kpool_tail_slot_mapping_kernel(
    ...
    # GLM_KPOOL_FIX (glm-kpool-audit): one launch, writes the whole mapping into
    # the persistent builder buffer. Token i of a real query interval
    # [qsl[0], qsl[num_reqs]) with a valid generic slot maps to
    # own_block(req) * RING + pos % RING; every other entry (graph padding,
    # PAD_SLOT_ID) is copied through unchanged, so padded rows stay -1.
    i = tl.program_id(0) * BLOCK + tl.arange(0, BLOCK)
    m = i < n
    s_in = tl.load(slot_in_ptr + i, mask=m, other=-1)
    lo = tl.load(qsl_ptr)
    hi = tl.load(qsl_ptr + num_reqs)
    # searchsorted(qsl, i, right=True) - 1 for i >= qsl[0]: #{r >= 1 : qsl[r] <= i}
    req = tl.zeros([BLOCK], dtype=tl.int32)
    for r in range(1, num_reqs + 1):
        req += (tl.load(qsl_ptr + r) <= i).to(tl.int32)
    req = tl.minimum(req, num_reqs - 1)
    real = m & (i >= lo) & (i < hi) & (i < num_actual_tokens) & (s_in >= 0)
    own = tl.load(
        block_table_ptr + req.to(tl.int64) * block_table_stride, mask=real, other=0
    ).to(tl.int64)
    pos = tl.load(positions_ptr + i, mask=real, other=0).to(tl.int64)
    rem = pos % RING
    rem = tl.where(rem < 0, rem + RING, rem)  # torch.remainder semantics
    tl.store(out_ptr + i, tl.where(real, own * RING + rem, s_in).to(s_in.dtype), mask=m)


def compute_kpool_tail_slot_mapping_reference(
    slot_mapping: torch.Tensor,
    block_table: torch.Tensor,
    query_start_loc: torch.Tensor,
    ...
    """Torch reference of the kernel above (tests only): the image's mapping
    restricted to real query tokens with a valid generic slot."""
    out = slot_mapping.clone()
    n = slot_mapping.numel()
    if n == 0 or num_reqs == 0:
        return out
    device = slot_mapping.device
    tokens = torch.arange(n, device=device)
    qsl = query_start_loc[: num_reqs + 1].to(torch.int64)
    req = torch.searchsorted(qsl, tokens, right=True) - 1
    req = req.clamp_(min=0, max=num_reqs - 1)
    real = (
        (tokens >= qsl[0])
        & (tokens < qsl[num_reqs])
        & (tokens < num_actual_tokens)
        & (slot_mapping >= 0)
    )
    own_block = block_table[:num_reqs, 0].index_select(0, req).to(torch.int64)
    pos = positions[:n].to(torch.int64)
    val = own_block * kpool + torch.remainder(pos, kpool)
    return torch.where(real, val, out)
...
def compute_kpool_tail_slot_mapping(
    ...
    pos % bs``. The tail group's block table only has its FIRST column written
    (one block per request), so without this every token at ``pos >= ring``
    collapses onto tail block 0 and concurrent requests share one ring.

    GLM_KPOOL_FIX: ``out`` is the builder's persistent buffer (the indexer op is
    captured into the FULL decode graph, which bakes the address in; a per-step
    clone dangles, cf. vcruz305's Xid 13). One Triton launch, no temporaries.
    Graph padding and PAD_SLOT_ID entries are preserved.
    """
    if out is None:
        return compute_kpool_tail_slot_mapping_reference(
            slot_mapping, block_table, query_start_loc, positions,
            num_actual_tokens, num_reqs, kpool,
        )
    n = slot_mapping.numel()
    if n == 0:
        return out
    if num_reqs == 0:
        ...
        return out
    BLOCK = 256
    _kpool_tail_slot_mapping_kernel[(triton.cdiv(n, BLOCK),)](
        slot_mapping,
        out,
        block_table,
...
class KpoolTailMetadataBuilder(AttentionMetadataBuilder):
    ...
    def __init__(
        ...
        # Persistent: the indexer op is captured into the FULL decode graph,
        # which bakes this tensor's address in (a per-step clone dangles).
    ...
    def build(
        ...
        if positions is not None:
            ...
            slot_mapping = compute_kpool_tail_slot_mapping(
                ...
                out=self.slot_mapping_buffer[: slot_mapping.numel()].view_as(
                    slot_mapping
                ),
            )
